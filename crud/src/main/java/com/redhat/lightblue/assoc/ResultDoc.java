...
/**
 * A result document of a particular entity type. This contains the document,
 * the unique id, the query plan node corresponding to this document, and the
 * references to other documents. During construction, all reference instances
 * are collected, and a DocReference list is constructed using those references.
 * Subsequent stages of execution attach new documents to those references.
 */
public class ResultDoc implements Serializable {
    ...
    /**
     * Sets the parent document reference of this document that is coming from
     * parentNode
     */
     ...
     * references corresponding to those destination nodes, and initializes a
     * reference for each
     */
    ...
    /**
     * This is used to associate the documents when a child document is searched
     * before the parent document. When this happens, each reference of the
     * parent document is looked up in child doc list using the edge criteria.
     *
     * @param parentDocs Docs that contain the childDocs. All parent docs in
     * this list should be of the same entity type
     * @param childDocs Docs that are contained in parent docs. All child docs
     * in this list should be of the same entity type
     *
     * childDocs must be all from the same query plan node. parentDocs must be
     * all from the same query plan node. The query plan nodes for child and
     * parent are different, and linked with edgeClauses
     */
