...
/**
 * This class represents a field binding interpreted based on the metadata. It
 * contains the original field binding information, the field type information,
 * and the field name relative to an entity in a query plan that denotes the
 * field name containing the value that is bound in the field binding.
 *
 * For example:
 * <pre>
 ...
 * </pre>
 *
 * Assume <code>referencedField</code> is bound to a value in a different
 * entity. Then, <code>binding</code> contains the binding information for
 * <code>referencedField</code>, <code>type</code> contains the type of
 * <code>referencedField</code>, and valueField contains
 * <code>referencedField</code>, the field relative to the entity it is
 * contained in. If the binding is a list binding, then type keeps the type of
 * the list element.
 */
public class ResolvedFieldBinding implements Serializable {
    ...
    /**
     * This class is used to return binding results. It contains the bindings,
     * and the query rewritten relative to the node the query is bound,
     */
    ...
    /**
     * Construct a resolved field binding using the given binding and the root
     * composite metadata
     */
    ...
    public static class RelativeRewriter extends MapQueryFieldsIterator {
         ...
         * @param thisMd Ths metadata of the node relative to which the query
         * will be rewritten
         * @param requestQuery if the query is a request query, this is true. If
         * the query is associated to a query plan node, this is false.
         */
        ...
        /**
         * Array match expressions require special treatment when converting a
         * query to relative. Relative query may require removing the array
         * clause to open up the elemMatch clause. This is the case if, for
         * instance, the clause is { array: a.b.c, elemMatch:{ } }, and a.b.c is
     ...
     *
     * @return Returns the binding result containing the bindings, and the query
     * expression rewritten to be run at the given query plan node. Returns null
     * if there are no bindings or query.
     */
    ...
    /**
     * Returns the field to bind. One of the fields belong to the current query
     * plan node (atNode), and the other one belongs to an ancestor of the
     * current node, so we bind the field that belongs to an ancestor.
     */
    ...
    /**
     * Attempts to refresh this binding starting at the given parent document,
     * and ascending to its parents
     */
