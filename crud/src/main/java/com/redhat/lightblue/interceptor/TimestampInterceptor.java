...
import com.redhat.lightblue.metadata.TimestampFields;

public class TimestampInterceptor implements CRUDDocInterceptor, MediatorInterceptor {

    public static final int TIMESTAMP_SEQ = 501;

    public void register(InterceptorManager mgr) {
        mgr.registerInterceptor(TIMESTAMP_SEQ, this, InterceptPoint.PRE_CRUD_INSERT_DOC,
    ...
    public void run(CRUDOperationContext ctx, DocCtx doc) {
        TimestampFields.initializeTimestampFields(ctx.getFactory().getNodeFactory(), ctx.getEntityMetadata(ctx.getEntityName()), doc);
    }
}
