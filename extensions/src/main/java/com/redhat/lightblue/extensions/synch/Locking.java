...
public interface Locking {
     ...
     * resourceId but with different callerIds, only one of them will
     * return success, and all others will fail. Subsequent calls to
     * acquire by different callers will fail until the lock owner
     * releases the lock. If the lock owner calls acquire, matching
     * number of release calls must be submitted to release the lock.
     *
     ...
     * lock must be acquired by the <code>callerId</code> before. A
     * release call decreases the acquire count of the lock, and when
     * that count reaches zero, the resource is no longer locked.
     *
     ...
     * <code>callerId</code> owns the lock for the resource
