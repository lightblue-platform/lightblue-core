...
public class IndexCaseInsensitiveTest {
    ...
    public void testIsCaseInsensIndex() {
        assertTrue(emd.getEntityInfo().getIndexes().getIndexes().stream().anyMatch(i -> i.isCaseInsensitiveKey(new Path("f1"))));
        assertFalse(emd.getEntityInfo().getIndexes().getIndexes().stream().anyMatch(i -> i.isCaseInsensitiveKey(new Path("f2"))));
        assertTrue(emd.getEntityInfo().getIndexes().getIndexes().stream().anyMatch(i -> i.isCaseInsensitiveKey(new Path("f3"))));
        assertFalse(emd.getEntityInfo().getIndexes().getIndexes().stream().anyMatch(i -> i.isCaseInsensitiveKey(new Path("f4"))));
    }
