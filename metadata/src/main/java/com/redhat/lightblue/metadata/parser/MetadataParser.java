...
public abstract class MetadataParser<T> {
        ...
        try {
            if (object != null) {
                ...
                if (null != fields && !fields.isEmpty()) {
                    ...
                    Error.get(Constants.ERR_PARSE_MISSING_ELEMENT, STR_FIELDS);
                }
        ...
        try {
            if (object != null) {
                ...
                if (null != values && !values.isEmpty()) {
                    ...
                    Error.get(Constants.ERR_PARSE_MISSING_ELEMENT, STR_VALUES);
                }
            ...
            if (logList != null) {
                for (T log : logList) {
                    ...
                    try {
                        ...
                        throw Error.get(Constants.ERR_ILL_FORMED_METADATA, d);
                    }
    ...
    public void parseEntityConstraints(EntitySchema schema,
        ...
        if (constraintList != null) {
            ...
            for (T x : constraintList) {
                ...
                String name = getSingleFieldName(x, Constants.ERR_INVALID_CONSTRAINT);
                Error.push(name);
                try {
                    EntityConstraintParser<T> parser = getEntityConstraintParser(name);
                    if (parser == null) {
                        throw Error.get(Constants.ERR_INVALID_CONSTRAINT, name);
                    }
    ...
    public DataStore parseDataStore(T object) {
        if (object != null) {
            String name = getSingleFieldName(object, Constants.ERR_INVALID_DATASTORE);
            DataStoreParser<T> p = getDataStoreParser(name);
            if (p == null) {
                throw Error.get(Constants.ERR_UNKNOWN_DATASTORE, name);
            }
    ...
    private Field parseSimpleField(String name,
        ...
        if (t == null) {
            throw Error.get(Constants.ERR_INVALID_TYPE, type);
        }
        field.setType(t);
        return field;
    }
    ...
    private Field parseReferenceField(String name,
        ...
        try {
            ...
            throw Error.get(Constants.ERR_ILL_FORMED_METADATA, e.toString());
        }
        return field;
    }
    ...
    private ArrayElement parseArrayItem(T items) {
        ...
        if (type.equals(ObjectType.TYPE.getName())) {
            ...
            throw Error.get(Constants.ERR_INVALID_ARRAY_ELEMENT_TYPE, type);
        } else {
        ...
        if (constraints != null && !constraints.isEmpty()) {
            ...
            try {
                ...
                for (FieldConstraint constraint : constraints) {
                    ...
                    if (parser == null) {
                        throw Error.get(Constants.ERR_INVALID_CONSTRAINT, constraintType);
                    }
    ...
    public void convertDataStore(T parent, DataStore store) {
        ...
        try {
            ...
            if (parser == null) {
                throw Error.get(Constants.ERR_UNKNOWN_DATASTORE, type);
            }
            T dsNode = newNode();
    ...
    private MetadataStatus statusFromString(String status) {
        switch (status) {
            ...
            default:
                throw Error.get(Constants.ERR_PARSE_INVALID_STATUS, status);
        }
    }
    ...
    public String getRequiredStringProperty(T object, String name) {
        ...
        try {
            ...
            if (property == null || property.trim().length() == 0) {
                throw Error.get(Constants.ERR_PARSE_MISSING_ELEMENT, name);
            }
            return property;
        } finally {
            Error.pop();
